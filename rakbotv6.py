#!/usr/bin/env python3
"""
rbx_bot.py — Telegram-бот для Roblox (v5).
pyTeleBot (telebot). БЕЗ aiogram.

v5:
  - Все типы прокси: http, https, socks4, socks4a, socks5, socks5h
  - До 10 прокси одновременно (round-robin + auto-failover)
  - PySocks проверяется только если есть SOCKS прокси
...
Поддерживаемые прокси:
    http://     - HTTP прокси
    https://    - HTTPS прокси
    socks4://   - SOCKS4 (нужен PySocks)
    socks4a://  - SOCKS4A, DNS на прокси (нужен PySocks)
    socks5://   - SOCKS5 (нужен PySocks)
    socks5h://  - SOCKS5, DNS на прокси (нужен PySocks)

Установка:
    pip install pyTelegramBotAPI requests
    pip install PySocks  # только для SOCKS прокси!

Запуск:
    export BOT_TOKEN="ваш-токен"
    export PROXY_URL="socks5://u:p@ip1:1080,https://u:p@ip2:443"
    python3 rbx_bot.py
"""
...
# ─────────────────────────────────────────────
# МУЛЬТИ-ПРОКСИ С РОТАЦИЕЙ (до 10, все типы)
# ─────────────────────────────────────────────
...
class ProxyPool:
    ...
    Протоколы: http, https, socks4, socks4a, socks5, socks5h.
    Можно миксовать разные типы в одном пуле.
    """
    ...
    def __init__(self, raw: str, max_proxies: int = 10):
        ...
        # Валидация схем
        valid = []
        for p in parts[:max_proxies]:
            scheme = p.split("://")[0].lower() if "://" in p else ""
            if scheme in SUPPORTED_SCHEMES:
                valid.append(p)
            else:
                log.warning(
                    "Неизвестный протокол: %s (поддерживаем: %s)",
                    p[:40], ", ".join(SUPPORTED_SCHEMES)
                )
        self._proxies = valid

        # Проверяем PySocks только если есть SOCKS прокси
        socks_proxies = [
            p for p in self._proxies
            if p.split("://")[0].lower() in SOCKS_SCHEMES
        ]
        if socks_proxies:
            try:
                ...
                schemes = set(
                    p.split("://")[0].lower() for p in socks_proxies
                )
                print()
                print("=" * 55)
                print("  PySocks не установлен!")
                print(f"  Требуется для: {', '.join(schemes)}")
                print("  pip install PySocks")
                print()
                print("  HTTP/HTTPS прокси работают без PySocks.")
                print("=" * 55)
                sys.exit(1)
        ...
        if self._proxies:
            ...
            for i, p in enumerate(self._proxies):
                scheme = p.split("://")[0] if "://" in p else "?"
                display = p.split("@")[-1] if "@" in p else p.split("://")[-1]
                log.info("  [%d] %s://%s", i + 1, scheme, display)
