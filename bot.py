...
BYPASS_LUFS = float(os.environ.get("BYPASS_LUFS", "-5"))
BYPASS_PEAK_DB = float(os.environ.get("BYPASS_PEAK_DB", "5"))
...
def process_track(item: dict):
    ...
    # Обложка качается параллельно с аудио и анализом: поллинг Pending-превью
    # (до ~15 сек) больше не блокирует пайплайн.
    with ThreadPoolExecutor(max_workers=1) as ex:
        fut_cover = ex.submit(fetch_thumbnail, item["id"])
        ogg = download_audio(item["id"])
        analysis = analyze_and_encode(ogg)
        cover = fut_cover.result()
...
def poller_loop():
    """Фоновый поток: проверяет новинки каждые CHECK_INTERVAL секунд.
    Работает независимо от воркера — очередь наполняется даже пока
    воркер занят обработкой тяжёлого трека."""
...
def main():
    ...
    log.info("started: poll every %ds, bypass at >%s LUFS or >%s dB peak", CHECK_INTERVAL, BYPASS_LUFS, BYPASS_PEAK_DB)
