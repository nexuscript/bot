...
def check_once(conn: sqlite3.Connection):
    ...
    for i in reversed(new_ids):
        ...
        try:
            ...
            log.exception("failed to process %s — will retry (attempt %d/%d)", i, get_attempts(conn, i), MAX_ATTEMPTS)
            time.sleep(3)
            continue  # не помечаем posted — попробуем ещё раз в следующем цикле
