#!/usr/bin/env python3
"""
rbx_bot.py — Telegram-бот для Roblox (v2).
pyTeleBot (telebot). БЕЗ aiogram.

Фиксы v2:
  - Друзья отображаются корректно (name/displayName)
  - Поиск игр работает (правильный API)
  - Игра по placeId (автоконверсия placeId → universeId)
  - Значки без HTTP 400 (правильные параметры)
  - Инлайн-кнопки пагинации (друзья, группы, значки)
...
# ─────────────────────────────────────────────
S = requests.Session()
S.headers.update({"Accept": "application/json", "User-Agent": "RBXInfoBot/2.0"})
...
def get_friends_count(user_id: int) -> int:
    try:
        return api_get(f"https://friends.roblox.com/v1/users/{user_id}/friends/count").get("count", 0)
    except Exception:
        return 0
...
def get_followers_count(user_id: int) -> int:
    try:
        return api_get(f"https://friends.roblox.com/v1/users/{user_id}/followers/count").get("count", 0)
    except Exception:
        return 0
...
def get_followings_count(user_id: int) -> int:
    try:
        return api_get(f"https://friends.roblox.com/v1/users/{user_id}/followings/count").get("count", 0)
    except Exception:
        return 0


def get_friends_list(user_id: int) -> list:
    """Получить ВСЕХ друзей (API отдаёт до 200)."""
...
def get_user_badges(user_id: int) -> list:
    """Получить значки с пагинацией (до 100)."""
    all_badges = []
    cursor = ""
    for _ in range(5):  # макс 5 страниц по 25
        params = {"limit": 25, "sortOrder": "Desc"}
        if cursor:
            params["cursor"] = cursor
        data = api_get(f"https://badges.roblox.com/v1/users/{user_id}/badges", params)
...
def place_to_universe(place_id: int) -> int:
    """Конвертировать placeId в universeId."""
    data = api_get(
...
def get_game_info(universe_id: int) -> dict:
    data = api_get("https://games.roblox.com/v1/games", {"universeIds": str(universe_id)})
    games = data.get("data", [])
    if not games:
...
def get_game_thumbnail(universe_id: int) -> Optional[str]:
    try:
        data = api_get(
            ...
            {"universeIds": str(universe_id), "size": "512x512", "format": "Png", "isCircular": "false"},
        )
        items = data.get("data", [])
        if items and items[0].get("imageUrl"):
            return items[0]["imageUrl"]
...
def search_games(keyword: str, limit: int = 10) -> list:
    """Поиск игр через Roblox search API."""
    try:
        # Используем games.roblox.com/v1/games/list
        data = api_get(
        ...
        # Fallback: через catalog search
        data2 = api_get(
            "https://www.roblox.com/games/list-json",
            {"keyword": keyword, "MaxRows": limit, "StartRows": 0},
        )
        return data2.get("Games", [])
    except Exception:
        pass

    # Fallback 2: через search API
    try:
        r = S.get(
            "https://apis.roblox.com/search-api/omni-search",
            params={"searchQuery": keyword, "searchType": "games", "pageToken": "", "sessionId": ""},
...
def get_asset_info(asset_id: int) -> dict:
    """Информация об ассете через economy API."""
...
ASSET_TYPES = {
    ...
    47: "Waist Accessory", 48: "Climb Animation",
    49: "Death Animation", 50: "Fall Animation",
    51: "Idle Animation", 52: "Jump Animation",
    53: "Run Animation", 54: "Swim Animation",
    55: "Walk Animation", 56: "Pose Animation",
    61: "Emote Animation",
}

# Какие типы можно скачивать
...
# ─────────────────────────────────────────────
# ПАГИНАЦИЯ — INLINE КНОПКИ
# ─────────────────────────────────────────────
def make_page_keyboard(prefix: str, user_id: int, page: int, total_pages: int):
    """Создать клавиатуру ◀ страница ▶ ."""
    kb = types.InlineKeyboardMarkup(row_width=3)
    buttons = []

    if page > 0:
        buttons.append(types.InlineKeyboardButton("◀", callback_data=f"{prefix}:{user_id}:{page - 1}"))
    else:
...
HELP_TEXT = (
    ...
    "  /avatar &lt;ник&gt; — аватар\n"
    "  /headshot &lt;ник&gt; — хедшот\n"
    "  /friends &lt;ник&gt; — друзья (с пагинацией)\n"
    "  /groups &lt;ник&gt; — группы (с пагинацией)\n"
    "  /badges &lt;ник&gt; — значки (с пагинацией)\n"
    ...
    "📥 Скачивание: Shirt, Pants, T-Shirt, Decal, Audio, Model\n"
...
def cmd_ping(m):
    ...
    bot.edit_message_text(f"🏓 Pong! <b>{ms} мс</b>", m.chat.id, msg.message_id, parse_mode="HTML")
...
def cmd_profile(m):
    ...
    try:
        ...
        text = (
            ...
            f"├ ID: <code>{uid}</code>\n"
            f"├ Статус: {status}\n"
            f"├ Создан: {fmt_date(user.get('created', ''))} ({fmt_ago(user.get('created', ''))})\n"
            f"├ Бан: {banned}\n"
        ...
        if desc:
            desc = safe_html(desc[:300]) + ("..." if len(desc) > 300 else "")
            text += f"\n\n📝 <i>{desc}</i>"

        text += f"\n\n🔗 <a href=\"https://www.roblox.com/users/{uid}/profile\">Открыть на Roblox</a>"

        # Кнопки быстрого доступа
        kb = types.InlineKeyboardMarkup(row_width=3)
        kb.add(
            types.InlineKeyboardButton("👥 Друзья", callback_data=f"friends:{uid}:0"),
            types.InlineKeyboardButton("🏰 Группы", callback_data=f"groups:{uid}:0"),
            types.InlineKeyboardButton("🏅 Значки", callback_data=f"badges:{uid}:0"),
        )

        if av:
            bot.send_photo(m.chat.id, av, caption=text, parse_mode="HTML", reply_markup=kb)
        else:
            bot.send_message(m.chat.id, text, parse_mode="HTML", disable_web_page_preview=True, reply_markup=kb)
...
def cmd_avatar(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2:
        return bot.reply_to(m, "❌ <code>/avatar Roblox</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
    try:
...
def cmd_headshot(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2:
        return bot.reply_to(m, "❌ <code>/headshot Roblox</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
    try:
...
# ─────────────────────────────────────────────
# /friends — с пагинацией
# ─────────────────────────────────────────────
def build_friends_text(user_id: int, page: int):
    """Построить текст страницы друзей."""
    ...
    lines = [f"👥 <b>Друзья</b> (всего {fmt_num(total_count)}), стр. {page + 1}/{total_pages}:\n"]

    for i, f in enumerate(page_items, start=page * PAGE_SIZE + 1):
        # Правильные поля API: name, displayName, isOnline, hasVerifiedBadge, id
        display = safe_html(f.get("displayName") or f.get("name") or "???")
        uname = safe_html(f.get("name") or "???")
        is_online = f.get("isOnline", False)
        verified = " ✅" if f.get("hasVerifiedBadge") else ""
        dot = "🟢" if is_online else "⚫"
        fid = f.get("id", "?")

        lines.append(f"  {i}. {dot} <b>{display}</b> (<code>{uname}</code>) ID:{fid}{verified}")
...
def cmd_friends(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2:
        return bot.reply_to(m, "❌ <code>/friends Roblox</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
    try:
...
def build_groups_text(user_id: int, page: int):
    ...
    lines = [f"🏰 <b>Группы</b> (всего {len(groups)}), стр. {page + 1}/{total_pages}:\n"]
...
def cmd_groups(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2:
        return bot.reply_to(m, "❌ <code>/groups Roblox</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
    try:
...
# /badges — с пагинацией
# ─────────────────────────────────────────────
# Кэш значков чтобы не запрашивать повторно при перелистывании
_badges_cache: dict[int, list] = {}
_badges_cache_time: dict[int, float] = {}


def get_cached_badges(user_id: int) -> list:
    now = time.time()
    if user_id in _badges_cache and now - _badges_cache_time.get(user_id, 0) < 120:
        return _badges_cache[user_id]
    badges = get_user_badges(user_id)
    _badges_cache[user_id] = badges
    _badges_cache_time[user_id] = now
    return badges


def build_badges_text(user_id: int, page: int):
    badges = get_cached_badges(user_id)
    if not badges:
        return "🏅 Нет значков", None

    page_items, page, total_pages = paginate(badges, page)

    lines = [f"🏅 <b>Значки</b> (всего {len(badges)}), стр. {page + 1}/{total_pages}:\n"]
    ...
    for i, b in enumerate(page_items, start=page * PAGE_SIZE + 1):
        ...
        awarded = fmt_date(b.get("awarded") or b.get("created", ""))
        lines.append(
            f"  {i}. <b>{bname}</b>\n"
            f"      └ {bdesc}"
        )
...
def cmd_badges(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2:
        return bot.reply_to(m, "❌ <code>/badges Roblox</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
    try:
...
def cmd_online(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2:
        return bot.reply_to(m, "❌ <code>/online Roblox</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
    try:
        ...
        p = presences[0]
        text = (
            f"👤 <b>{safe_html(user.get('displayName', '?'))}</b> "
            f"(@{safe_html(user.get('name', '?'))})\n\n"
        ...
        if root:
            text += f"\n🔗 <a href=\"https://www.roblox.com/games/{root}\">Открыть игру</a>"
...
def cmd_search(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2:
        return bot.reply_to(m, "❌ <code>/search Builderman</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
    try:
        ...
        for i, u in enumerate(results, 1):
            ...
            lines.append(f"  {i}. <b>{dn}</b> (<code>{un}</code>) ID:{u.get('id', '?')}{v}")
...
# ─────────────────────────────────────────────
# /game — поддержка placeId и universeId
# ─────────────────────────────────────────────
...
def cmd_game(m):
    ...
    if len(args) < 2 or not args[1].strip().isdigit():
        return bot.reply_to(
            m,
            "❌ Укажи ID игры (placeId или universeId)\n"
            "<code>/game 286090429</code>\n"
            "💡 placeId из URL: roblox.com/games/<b>286090429</b>/...",
            parse_mode="HTML",
        )
    cd = check_cooldown(m.from_user.id)
    if cd:
    ...
    try:
        ...
        try:
            ...
            # Не нашли — пробуем как placeId → конвертируем
            try:
                ...
                raise RBXError(
                    ...
                    "Попробуй другой ID или /searchgame"
                )
        ...
        text = (
            ...
            f"├ 🟢 Играют: <b>{fmt_num(game.get('playing', 0))}</b> / {game.get('maxPlayers', '?')}\n"
        ...
        if root:
            text += f"\n\n🔗 <a href=\"https://www.roblox.com/games/{root}\">Открыть на Roblox</a>"

        if thumb:
            bot.send_photo(m.chat.id, thumb, caption=text, parse_mode="HTML")
        else:
            bot.send_message(m.chat.id, text, parse_mode="HTML", disable_web_page_preview=True)
        bot.delete_message(m.chat.id, w.message_id)

    except RBXError as e:
        bot.edit_message_text(f"❌ {e}", m.chat.id, w.message_id, parse_mode="HTML")
    except Exception as e:
...
def cmd_searchgame(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2:
        return bot.reply_to(m, "❌ <code>/searchgame Adopt Me</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
    ...
    try:
        ...
        if not games:
            return bot.edit_message_text("🔍 Ничего не найдено", m.chat.id, w.message_id)

        lines = [f"🎮 Поиск <b>{safe_html(args[1])}</b>:\n"]
        for i, g in enumerate(games, 1):
            # Разные API отдают разные поля
            ...
            pid = g.get("placeId") or g.get("PlaceId") or ""
            lines.append(
...
def cmd_group(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2 or not args[1].strip().isdigit():
        return bot.reply_to(m, "❌ <code>/group 1</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
    try:
        ...
        text = (
            ...
            f"├ 🔓 Открытая: {'✅' if g.get('publicEntryAllowed') else '❌'}\n"
            f"└ 🔒 Заблокирована: {'🔒 Да' if g.get('isLocked') else '✅ Нет'}"
        )
        ...
        text += f"\n\n🔗 <a href=\"https://www.roblox.com/groups/{gid}\">Открыть на Roblox</a>"
        bot.send_message(m.chat.id, text, parse_mode="HTML", disable_web_page_preview=True)
    except RBXError as e:
        bot.reply_to(m, f"❌ {e}")
...
def cmd_asset(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2 or not args[1].strip().isdigit():
        return bot.reply_to(m, "❌ <code>/asset 607785314</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
    ...
    try:
        ...
        text = (
            ...
            f"├ 👤 Автор: <b>{safe_html(creator.get('Name', '?'))}</b>\n"
            f"├ {price_str}\n"
            f"├ 🛒 В продаже: {'✅' if asset.get('IsForSale') else '❌'}\n"
            f"├ 📊 Продаж: <b>{fmt_num(asset.get('Sales', 0))}</b>{limited}\n"
        ...
        text += f"\n\n🔗 <a href=\"https://www.roblox.com/catalog/{aid}\">Открыть на Roblox</a>"

        # Кнопка скачивания
        kb = None
        if type_id in DOWNLOADABLE:
            kb = types.InlineKeyboardMarkup()
            kb.add(types.InlineKeyboardButton("📥 Скачать", callback_data=f"dl:{aid}"))

        if thumb:
            bot.send_photo(m.chat.id, thumb, caption=text, parse_mode="HTML", reply_markup=kb)
        else:
...
# ─────────────────────────────────────────────
# /download — скачать ассет
# ─────────────────────────────────────────────
@bot.message_handler(commands=["download"], func=private_only)
def cmd_download(m):
    args = m.text.split(maxsplit=1)
    if len(args) < 2 or not args[1].strip().isdigit():
        return bot.reply_to(m, "❌ <code>/download 607785314</code>", parse_mode="HTML")
    cd = check_cooldown(m.from_user.id)
    if cd:
        return bot.reply_to(m, f"⏳ Подожди {cd} сек.")
...
def do_download(chat_id: int, asset_id: int, callback_msg_id: int = None):
    """Скачать и отправить ассет."""
    w = bot.send_message(chat_id, f"📥 Скачиваю ассет {asset_id}...")
    try:
        # Получим инфо чтобы знать тип
        try:
        ...
        if not data:
            return bot.edit_message_text("❌ Не удалось скачать ассет", chat_id, w.message_id)

        ext = ASSET_EXT.get(type_id, ".bin")
        type_name = ASSET_TYPES.get(type_id, "Unknown")

        # Для clothing (Shirt/Pants/T-Shirt) — извлечь текстуру URL
        if type_id in (2, 11, 12):
            # XML содержит url текстуры
            ...
            if url_match:
                ...
                if "rbxassetid://" in texture_url:
                    ...
                    if tex_id:
                        texture_url = f"https://assetdelivery.roblox.com/v1/asset/?id={tex_id.group(1)}"

                try:
                    tr = S.get(texture_url, timeout=30, allow_redirects=True)
                    if tr.status_code == 200 and len(tr.content) > 100:
                        data = tr.content
                        ext = ".png"
                except Exception:
                    pass  # отправим XML если не удалось

        # Для Decal — извлечь изображение
        if type_id == 13:
            text_content = data.decode("utf-8", errors="ignore")
            url_match = re.search(r'<url>([^<]+)</url>', text_content)
            if not url_match:
        ...
        # Для Audio — определить расширение по magic bytes
        if type_id == 3:
            if data[:4] == b'OggS':
                ext = ".ogg"
        ...
        caption = f"📥 <b>{safe_html(name)}</b>\n├ Тип: {type_name}\n├ ID: <code>{asset_id}</code>\n└ Размер: {len(data) // 1024} КБ"

        # Отправить audio как аудио
        if type_id == 3 and ext == ".ogg":
            bot.send_audio(chat_id, file_obj, caption=caption, parse_mode="HTML")
        elif ext == ".png" and len(data) < 10 * 1024 * 1024:
            # Отправить как фото + документ
            try:
                bot.send_photo(chat_id, io.BytesIO(data), caption=caption, parse_mode="HTML")
            except Exception:
                file_obj.seek(0)
                bot.send_document(chat_id, file_obj, caption=caption, parse_mode="HTML")
        else:
            bot.send_document(chat_id, file_obj, caption=caption, parse_mode="HTML")
...
# ─────────────────────────────────────────────
# CALLBACK QUERIES — пагинация + скачивание
# ─────────────────────────────────────────────
...
def handle_callback(call):
    ...
    if parts[0] == "dl" and len(parts) == 2:
        ...
        threading.Thread(target=do_download, args=(call.message.chat.id, asset_id), daemon=True).start()
        return

    # Пагинация: friends:USER_ID:PAGE
    if len(parts) == 3:
        section = parts[0]
        try:
            user_id = int(parts[1])
        ...
        try:
            ...
            if "message is not modified" in str(e):
                bot.answer_callback_query(call.id, "Уже на этой странице")
            else:
...
# Неизвестные команды
# ─────────────────────────────────────────────
@bot.message_handler(func=lambda m: m.text and m.text.startswith("/") and private_only(m))
def cmd_unknown(m):
...
if __name__ == "__main__":
    log.info("=" * 40)
    log.info("  RBX Info Bot v2 запущен!")
    log.info("  Cooldown: %ds | Page size: %d", COOLDOWN_SEC, PAGE_SIZE)
    log.info("=" * 40)
