...
BOT_TOKEN = "8704051951:AAEZRKWjvd6BPfdFhoWNyY6utNem9JlLPv8"
COOLDOWN_SECONDS = 60
MIN_LEVEL_UP = 1
MAX_LEVEL_UP = 10
DB_FILE = "curator.db"
...
SPAM_LIMIT = 5        # 5 команд
SPAM_WINDOW = 10       # за 10 секунд
SPAM_BAN_TIME = 60     # бан на 60 секунд

# ======================== 100 РАНГОВ ========================

RANKS = [
    (0, "🌱 Новичок"), (10, "📝 Стажёр"), (25, "📎 Младший куратор"),
    (50, "📋 Куратор"), (100, "⭐ Старший куратор"), (200, "🔥 Мастер-куратор"),
    ...
    (15_000_000_000_000, "♾️ За пределами"),
]
    ...
    async def __call__(self, handler, event, data):
        ...
        if uid and check_spam(uid):
            if isinstance(event, Message):
                ...
                if ban_left > 0:
                    try:
                        await event.reply(f"🚫 Антиспам! Подождите <b>{ban_left}с</b>")
                    except:
                        pass
            elif isinstance(event, CallbackQuery):
                try:
                    await event.answer("🚫 Антиспам!", show_alert=True)
                except:
                    pass
            return
...
async def cmd_curator(message: Message):
    ...
    if key in pending_captchas:
        if time.time() - pending_captchas[key]["created"] < CAPTCHA_TIMEOUT:
            await message.reply("⚠️ Решите капчу выше!")
            return
