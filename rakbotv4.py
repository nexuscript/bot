...
if PROXY_URL:
    # Проверяем PySocks для SOCKS5 прокси
    if PROXY_URL.startswith("socks"):
        try:
            ...
            log.critical(
                "\\n" + "=" * 50 +
                "\\n  PySocks не установлен!" +
                "\\n  SOCKS5 прокси требует библиотеку PySocks:" +
                "\\n  pip install PySocks" +
                "\\n" + "=" * 50
            )
            sys.exit(1)
...
def api_get(url, params=None):
    try:
        ...
        if "Missing dependencies for SOCKS" in str(e):
            raise RBXError("PySocks не установлен! pip install PySocks")
        raise RBXError(f"Ошибка подключения: {e}")
